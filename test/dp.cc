...
int RANDOM_SEED = 42;
float VIDEO_CHUNCK_LEN = 2000;  // (ms), every time add this amount to buffer
unsigned int BITRATE_LEVELS = 6;
unsigned int TOTAL_VIDEO_CHUNCK = 157;
float PACKET_PAYLOAD_PORTION = 0.95;
unsigned int LINK_RTT = 54;  // millisec
unsigned int PACKET_SIZE = 1500;  // bytes
float DT = 0.05;  // time granularity
float BUFFER_THRESH = 60;  // sec, max buffer limit
static const int VIDEO_BIT_RATE[] = {20000, 40000, 60000, 80000, 110000, 160000};  // Kbps
unsigned int DEFAULT_QUALITY = 1;
unsigned int MAX_QUALITY = 5;
float REBUF_PENALTY = 160;  // 1 sec rebuffering -> 3 Mbps
